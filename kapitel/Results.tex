...
The previous compariosons covered FD001 (simplest), FD002(multi-condition), a pairwise combination and the full dataset. To round out the picture, this sectionlooks at FD004, the most complex subset, with six operating conditions and two fault modes, and the FD001-FD004 pairing, which puts the simplest and most complex subset together in a single training set.

%%%Continue here figure

On FD004 alone, s11 leads for the consensus methods (IG, SHAP, FA, ALE), just as it did on FD002. That is not surprising, both share six operating conditions and s11 appears to carry the most reliable degradation signal when operating-point variablility is high. Sensors s9 and s12 hodl the second tier. What is worth noting is that FD004's two fault modes create a richer training set where no single sensor'spermutation causes the kind of catastrophic error spikes seen on FD002. CF remains diffuse, with no strong preference for any particular sensor.

Comparing FD004 single against FD001 single directly, the contrast is stark. On FD001, s9 dominates and the consensus methods assign it roughly double the importance of any other sensors. The model trained on teh more complex data has learned to spread its reliance across multiple signals rather tahn leanin heavily on one. A pattern that makes sense when operating conditions vary, since any single sensor's value becomes a less clean indicator of degradation state.

%% continue here figure

The pairwise FD001-FD004 model is interesting because it forces the network to reconcile the simples and most complex subsets. The resulting ranking is close to what the all-datamodel produces: s11, s9 and s12 as the top three, with s4 and s15 following. The consensus methods agree more tightly here than in either single-subset case. This suggests that exposure to both extremes of the complexity spectrum is enough to stabilise the learned feature representation, adding the intermediate subsets (FD002, FD003) contributes less additional information than one might expect. LIME continues to pile importance onto s11 alone and PFI deviates on different sensors than it did on the single models, but neither disrupts the consensus top three.
...
\subsection{Cross-Confguration Ranking Agreement}
